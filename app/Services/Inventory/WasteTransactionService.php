...
class WasteTransactionService
{
    ...
    /**
     * Create a transaction (sale or processing).
     *
     * @param  array{
     *   bank_sampah_id: int,
     *   offtaker_id: int,
     ...
     */
    public function createTransaction(array $data, Admin $admin): WasteTransaction
    {
        if ($data['type'] === 'processing') {
            return $this->createProcessing($data, $admin);
        }

        return $this->createSale($data, $admin);
    }
    ...
    public function createProcessing(array $data, Admin $admin): WasteTransaction
    {
        return DB::transaction(function () use ($data, $admin) {
            ...
            foreach ($data['items'] as $item) {
                ...
                $itemTotal = $item['quantity'] * ($item['harga_jual'] ?? 0);

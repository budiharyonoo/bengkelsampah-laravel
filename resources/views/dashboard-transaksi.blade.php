    ...
    <body>
        ...
        <div class="container">
                        ...
                        <div class="filter-dropdown">
                            ...
                            <div id="bankDropdown" class="filter-dropdown-content">
                                ...
                                @foreach ($bankSampahList as $bank)
                                    <div class="filter-option" data-bank-name="{{ strtolower($bank->nama_bank_sampah) }}"
            ...
            <div class="controls" style="margin-top: 1rem;">
                <div class="search-filter">
                    <form method="GET" action="{{ route('dashboard.transaksi') }}" class="search-filter"
                        id="mainFilterForm">
                        ...
                        <div class="search-box">
                            <input type="text" name="search" class="search-input" placeholder="Cari transaksi..."
                        ...
                        <div class="filter-dropdown">
                            ...
                            <div id="tipeDropdown" class="filter-dropdown-content">
                                <div class="filter-option" onclick="selectFilter('tipe_setor', '', 'Tipe Transaksi')">
                                    Semua Tipe</div>
                                <div class="filter-option" onclick="selectFilter('tipe_setor', 'jual', 'Jual')">Jual</div>
                                <div class="filter-option" onclick="selectFilter('tipe_setor', 'sedekah', 'Sedekah')">
                                    Sedekah</div>
                                <div class="filter-option" onclick="selectFilter('tipe_setor', 'tabung', 'Tabung')">Tabung
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="filter-dropdown">
                            ...
                            <div id="statusDropdown" class="filter-dropdown-content">
                                <div class="filter-option" onclick="selectFilter('status', '', 'Status')">Semua Status
                                </div>
                                <div class="filter-option"
                                    onclick="selectFilter('status', 'dikonfirmasi', 'Dikonfirmasi')">Dikonfirmasi</div>
                                <div class="filter-option" onclick="selectFilter('status', 'diproses', 'Diproses')">
                                    Diproses</div>
                                <div class="filter-option" onclick="selectFilter('status', 'dijemput', 'Dijemput')">
                                    Dijemput</div>
                                <div class="filter-option" onclick="selectFilter('status', 'selesai', 'Selesai')">Selesai
                                </div>
                        ...
                        <a href="{{ route('dashboard.transaksi') }}" class="reset-btn"
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @forelse ($transaksi as $trx)
                                <tr>
                                    <td>#{{ $trx->id }}</td>
                                    <td>{{ $trx->created_at ? \Carbon\Carbon::parse($trx->created_at)->format('d/m/Y H:i') : '-' }}
                                    </td>
                                    <td>
                                        <div style="display: flex; flex-direction: column;">
                                            <span
                                                style="font-weight: 600; color: #1e293b;">{{ $trx->user_name ?? '-' }}</span>
                                            <span
                                                style="font-size: 12px; color: #6b7280; margin-top: 2px;">{{ $trx->user_identifier ?? '-' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span
                                            class="badge {{ $tipeClass }}">{{ ucfirst($trx->tipe_setor ?? '-') }}</span>
                                    </td>
                                    <td>
                                        @php
                                    ...
                                    <td class="action-cell">
                                        <a href="{{ route('dashboard.transaksi.show', $trx->id) }}"
                                            class="action-btn detail-btn" title="Detail">
                                        ...
                                        @if (Auth::guard('admin')->user()->role === 'admin')
                                            <button type="button" class="action-btn delete-btn" title="Hapus"
                                                ...
                                                <img src="{{ asset('icon/ic_delete.svg') }}" alt="Hapus">
                                            </button>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="9" class="no-results">Tidak ada data transaksi</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            @if (isset($transaksi) && $transaksi->hasPages())
                <div class="pagination-container">
                    <div class="pagination-info">
                        {{ $transaksi->firstItem() ?? 0 }} - {{ $transaksi->lastItem() ?? 0 }} of
                        {{ $transaksi->total() }}
                    </div>
                    <div class="pagination">
                        <button class="pagination-btn"
                            onclick="showLoading('Memuat halaman...'); window.location.href='{{ $transaksi->previousPageUrl() }}'"
                            {{ $transaksi->onFirstPage() ? 'disabled' : '' }}>&lt;</button>
                        <button class="pagination-btn"
                            onclick="showLoading('Memuat halaman...'); window.location.href='{{ $transaksi->nextPageUrl() }}'"
                            {{ !$transaksi->hasMorePages() ? 'disabled' : '' }}>&gt;</button>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="modal" id="exportExcelModal" style="display:none;">
            <div class="modal-content">
                ...
                <div class="modal-buttons">
                    ...
                    <button class="modal-button confirm-button" type="button" id="confirmExportExcelBtn">Export
                        Excel</button>
                </div>
            </div>
        </div>
        ...
        <div class="modal" id="exportCsvModal" style="display:none;">
            <div class="modal-content">
                ...
                <p class="modal-subtitle">Export data lengkap (Summary, Bank Performa, Items by Volume, Detail Transaksi)
                </p>
                <div class="export-form">
                    <div class="form-group">
                        <label class="form-label">Bank Sampah</label>
                ...
                <div class="modal-buttons">
                    ...
                    <button class="modal-button confirm-button" type="button" id="confirmExportCsvBtn">Export
                        CSV</button>
                </div>
            </div>
        </div>
        ...
        <div class="modal" id="exportPdfModal" style="display:none;">
            <div class="modal-content">
                ...
                <div class="modal-buttons">
                    ...
                    <button class="modal-button confirm-button" type="button" id="confirmExportPdfBtn">Export
                        PDF</button>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            function deleteTransaction(id) {
                if (confirm('Apakah Anda yakin ingin menghapus transaksi ini?')) {
                    ...
                    fetch('{{ route('dashboard.setoran.bulk-delete') }}', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name=\"csrf-token\"]').getAttribute('content')
                            },
